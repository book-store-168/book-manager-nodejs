...
exports.register = async (req, res) => {
    try {
        ...
        const [result] = await pool.execute(
            'INSERT INTO users (email, password, name, role, status, deleted) VALUES (?, ?, ?, "USER", "INACTIVE", 0)',
            [email, hash, name]
        );
...
exports.updateMe = async (req, res, next) => {
    ...
    // LOG debug: xem có vào route + nhận body không
    console.log('[updateMe] method=', req.method, 'userId=', userId, 'body=', req.body);
    ...
    try {
        ...
        await conn.execute('UPDATE users SET name = ? WHERE id = ?', [name.trim(), userId]);

        const [has] = await conn.execute('SELECT id FROM profiles WHERE user_id = ? LIMIT 1', [userId]);
        if (has.length) {
            await conn.execute(
                'UPDATE profiles SET phone = ?, address = ? WHERE user_id = ?',
        ...
        return res.json({
            ...
            echo: { method: req.method, body: req.body } // để FE thấy chắc chắn
        });
    } catch (e) {
        await conn.rollback();
